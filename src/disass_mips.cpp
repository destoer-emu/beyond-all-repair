...
std::string disass_mips_default(Program& program, u64 addr, const Opcode &opcode,const Instr* instr)
{
    ...
    switch(instr->fmt)
    {
        case instr_type::imm_signed:
        {
            ...
            {
                return fmt::format("{} {}, 0x{:04x}",instr->name,REG_NAMES[opcode.rt],u16(opcode.imm));
            }

            return fmt::format("{} {}, {}, {}",instr->name,REG_NAMES[opcode.rt],REG_NAMES[opcode.rs],opcode.imm);
        }
        ...
        case instr_type::imm_unsigned:
        {
            ...
            return fmt::format("{} {}, {}, 0x{:04x}",instr->name,REG_NAMES[opcode.rt],REG_NAMES[opcode.rs],u16(opcode.imm));
        }

        case instr_type::imm_rs:
        {
            return fmt::format("{} {}, 0x{:04x}",instr->name,REG_NAMES[opcode.rs],u16(opcode.imm));
        }
        ...
        case instr_type::branch_rs_rt:
        {
            ...
            return fmt::format("{} {}, {}, {}",instr->name,REG_NAMES[opcode.rs],REG_NAMES[opcode.rt],loc_name(program,target));
        }

        case instr_type::branch_rs:
        {
            const u64 target = branch_addr(addr,opcode.imm);
            return fmt::format("{} {}, {}",instr->name,REG_NAMES[opcode.rs],loc_name(program,target));            
        }
        ...
        case instr_type::jump:
        {
            ...
            return fmt::format("{} {}",instr->name,loc_name(program,target));
        }

        case instr_type::store:
        {
            return fmt::format("{} {}, {}({})",instr->name,REG_NAMES[opcode.rt],opcode.imm,REG_NAMES[opcode.rs]);
        }

        case instr_type::store_float:
        {
            return fmt::format("{} $f{}, {}({})",instr->name,get_ft(opcode),opcode.imm,REG_NAMES[opcode.rs]);
        }
        ...
        {
            return fmt::format("{} {}, {}({})",instr->name,REG_NAMES[opcode.rt],u16(opcode.imm),REG_NAMES[opcode.rs]);
        }
        ...
        case instr_type::reg_rd_rs_rt:
        {
            ...
            {
                return fmt::format("{} {}, {}",instr->name,REG_NAMES[opcode.rd],REG_NAMES[opcode.rt]);
            }
        ...
        case instr_type::reg_rd_rt_rs:
        {
            ...
            {
                return fmt::format("{} {}, {}",instr->name,REG_NAMES[opcode.rd],REG_NAMES[opcode.rs]);
            }

            return fmt::format("{} {}, {}, {}",instr->name,REG_NAMES[opcode.rd],REG_NAMES[opcode.rt],REG_NAMES[opcode.rs]);
        }

        case instr_type::reg_rd:
        {
            return fmt::format("{} {}",instr->name,REG_NAMES[opcode.rd]);
        }

        case instr_type::reg_rs:
        {
            return fmt::format("{} {}",instr->name,REG_NAMES[opcode.rs]);
        }

        case instr_type::reg_rs_rt:
        {
            return fmt::format("{} {}, {}",instr->name,REG_NAMES[opcode.rs],REG_NAMES[opcode.rt]);
        }
        ...
        {
            return fmt::format("{} {}, {}",instr->name,REG_NAMES[opcode.rt],REG_NAMES[opcode.rd]);
        }

        case instr_type::shift:
        {
            const u32 sa = read_sa(opcode.op);    
            return fmt::format("{} {}, {}, {}",instr->name,REG_NAMES[opcode.rd],REG_NAMES[opcode.rt],sa);
        }
        ...
        case instr_type::float_rt_fs:
        {
            ...
            return fmt::format("{} {}, $f{}",instr->name,REG_NAMES[opcode.rt],fs);
        }
        ...
        case instr_type::float_fd_fs:
        {
            ...
            const u32 fs = get_fs(opcode);
            return fmt::format("{} $f{}, $f{}",instr->name,fd,fs);
        }
        ...
        case instr_type::float_fd_fs_ft:
        {
            ...
            return fmt::format("{} $f{}, $f{}, $f{}",instr->name,fd,fs,ft);     
        }
        ...
        case instr_type::float_fs_ft:
        {
            const u32 fs = get_fs(opcode);
            const u32 ft = get_ft(opcode);
            return fmt::format("{} $f{}, $f{}",instr->name,fs,ft);            
        }
...
std::string fmt_opcode(const Opcode& opcode)
{
    ...
    return fmt::format("op: 0x{:x}, rt: 0x{:x}, rs 0x{:x}, rd 0x{:x}, imm, 0x{:04x}, type: 0b{:06b}",opcode.op, opcode.rt, opcode.rs,
...
std::string disass_lui(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    return fmt::format("lui {}, 0x{:04x}",REG_NAMES[opcode.rt],u16(opcode.imm));
}
...
std::string disass_cache(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    return fmt::format("cache {}, {}({})",opcode.rt,opcode.imm,REG_NAMES[opcode.rs]);
}
...
std::string disass_jalr(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    if(opcode.rd == RA)
    {
        return fmt::format("jalr {}",REG_NAMES[opcode.rs]);
    }
...
std::string disass_bgezal(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    if(opcode.rs == 0)
    {
        return fmt::format("bal {}",loc_name(program,target));
    }

    else
    {
        return fmt::format("bgezal {}, {}",REG_NAMES[opcode.rs],loc_name(program,target));
    }
}
...
std::string disass_beq(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    {
        return fmt::format("b {}",loc_name(program,target));
    }

    else
    {
        return fmt::format("beq {}, {}, {}",REG_NAMES[opcode.rs],REG_NAMES[opcode.rt],loc_name(program,target));
    }
}
...
std::string disass_pref(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    return fmt::format("pref {}, {}({})",hint,u16(opcode.imm),REG_NAMES[opcode.rs]);
}
...
std::string disass_or(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    if(opcode.rs == 0)
    {
        return fmt::format("move {}, {}",REG_NAMES[opcode.rd],REG_NAMES[opcode.rt]);
    }

    else if(opcode.rt == 0)
    {
        return fmt::format("move {}, {}",REG_NAMES[opcode.rd],REG_NAMES[opcode.rs]);
    }
...
std::string disass_nor(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    if(opcode.rt == 0)
    {
        return fmt::format("not {}, {}",REG_NAMES[opcode.rd],REG_NAMES[opcode.rs]);
    }
...
std::string disass_li(Program& program, u64 addr, const Opcode& opcode)
{
    ...
    if(opcode.rs == 0)
    {
        return fmt::format("li {}, 0x{:04x}",REG_NAMES[opcode.rt],u16(opcode.imm));
    }
...
{
    return fmt::format("{} {}, {}, {}, {}",name,REG_NAMES[opcode.rs],REG_NAMES[opcode.rd],pos,size);
}
